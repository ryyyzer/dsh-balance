...
/** Keep the platform's own model id (trimmed). No fuzzy remapping: when
 *  upstream renames or adds models (e.g. "deepseek-flash"), usage stays under
 *  that exact id instead of being mis-attributed to a legacy key. */
...
async function fetchTodayUsage(token, signal) {
	...
	// Preferred display order (known models first); anything else follows,
	// heaviest spend first. Upstream renames/additions need no code change.
	...
	const rank = (model) => {
		const i = PREFERRED_ORDER.indexOf(model);
		return i === -1 ? PREFERRED_ORDER.length : i;
	};
	const list = [];
	const others = [];
	for (const [, rec] of models) {
		if (rank(rec.model) < PREFERRED_ORDER.length) list.push(rec);
		else others.push(rec);
	}
	list.sort((a, b) => rank(a.model) - rank(b.model));
	others.sort((a, b) => b.cost - a.cost || b.tokens - a.tokens);
...
function apply(ctx) {
	...
	connection.fetch.register({
		path: "/api/balance",
		methods: ["GET"],
		// required by dsh >= 0.1.5 (ignored by earlier engines)
