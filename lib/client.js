...
//
//   · N rows — today's per-model consumption, one row per model the
//     platform reports (no hardcoded model ids; via host /api/balance/today;
//     needs DEEPSEEK_USER_TOKEN configured)
...
`;
		...
		/** Model row view (any model id reported by the platform). */
		function modelRow(model, rec) {
			const tokens = Number(rec?.tokens) || 0;
			return {
				key: model,
				name: model,
				sub: `${fmtTokens(tokens)} tokens`,
				amount: fmtMoney(Number(rec?.cost) || 0)
			};
		}
		...
		function BalanceChip() {
			...
			const todayModels = todayReady ? (todayState.data?.models ?? []).map((r) => modelRow(r.model, r)) : null;
					...
					children: [
						...
						todayModels === null
							? (0, react_jsx_runtime.jsx)("div", {
								...
								: todayModels.length + todayOthers.length === 0
									? (0, react_jsx_runtime.jsx)("div", {
										className: "dsb-mrow",
										children: (0, react_jsx_runtime.jsx)("span", { className: "dsb-mrow-amt", children: "今日暂无消耗" })
									})
